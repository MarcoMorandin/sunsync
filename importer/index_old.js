                    ...
                    await axios
                        ...
                        .then(async (response) => {
                            ...
                            await axios.post(process.env.PREDICTION_URL, wsDataPrompt).then(async (res) => {
                                predicted_power = Math.abs(res.data.predictions[0])
                                await axios
                                    ...
                                    .then(async (wsInfo) => {
                                        ...
                                        await axios.get(wsInfo.data.url + tomorrow.toISOString().split('T')[0]).then(async (wsData) => {
                                            ...
                                            await axios.post(process.env.PREDICTION_URL, wsDataTomorrowPrompt).then(async (tom_pred) => {
                                                tomorrow_predicted_power = Math.abs(tom_pred.data.predictions[0])
                                                date.setUTCDate(date.getUTCDate() - 1)
                                                // await tomorrowPredictionEventHandler(date, tomorrow_predicted_power, el.installed_power)
                                                if (!data['error']) {
                                                    let dayData = {
                                                        _id: new ObjectId(),
                                                        time: new Date(date.toISOString()),
