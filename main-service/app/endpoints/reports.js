...
router.get(
    '/production',
    [
        query('year', 'year must be an int')
            .optional()
            .isInt({ min: 0, max: 4000 }),
        query('pvinfo_id', 'pvinfo_id must be a valid Mongo ObjectId')
            .optional()
            .isMongoId(),
        query(
            'aggregation',
            'aggregation must be a value between year, month and day',
        )
            .optional()
            .isIn(['year', 'month', 'all']),
    ],
...
router.get('/pvnumber', async (req, res) => {
    let data = await PvInfo.aggregate([{ $count: 'number_of_pv_systems' }]);
