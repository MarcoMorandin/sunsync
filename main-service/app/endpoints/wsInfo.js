...
router.get('/:wsinfo_id', tokenChecker, param("wsinfo_id").isMongoId(), async (req, res) => {
...
router.post('', tokenChecker, [
    ...
    res.status(200).json({"info" : "Operazione completata", "data": data}).send();
})

router.delete('/:wsinfo_id', tokenChecker, param("wsinfo_id").isMongoId(), async (req, res) => {
    if(req.user.role == 1)
        return res.status(401).json({ "401 Unauthorized": "You are not authorized"})
    ...
    let weatherStationNum = await WeatherStation.countDocuments({ _id: ObjectId.createFromHexString(req.params.wsinfo_id) })
    if (!weatherStationNum || weatherStationNum === 0){
        res.status(404).json({ "404 Not Found": "No weather station found with the given ID"})
        return;
    }

    let data = await PvSystem.find({ ws_id: ObjectId.createFromHexString(req.params.wsinfo_id) });
    if(data.length !== 0) {
        res.status(412).json({"412 Precondition Failed": "Linked to Pv System"});
        return;
    }
    let eliminated = await WeatherStation.deleteOne({ _id: ObjectId.createFromHexString(req.params.wsinfo_id) });
    await WeatherData.deleteMany({ "metadata.ws_id": ObjectId.createFromHexString(req.params.wsinfo_id) });
